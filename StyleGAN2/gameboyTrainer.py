...
def generate_examples(gen, epoch, n=50):
    ...
    for i in range(n):
        with torch.no_grad():
            ...
            noise = get_noise(1)
            img = gen(w, noise)
            if not os.path.exists(f'saved_examples/gameboy/epoch{epoch}'):
                os.makedirs(f'saved_examples/gameboy/epoch{epoch}')
            save_image(img * 0.5 + 0.5, f"saved_examples/gameboy/epoch{epoch}/img_{i}.png")
